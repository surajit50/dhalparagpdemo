...
import { Edit, FileText, Building2, IndianRupee, Hash, Filter, X, ArrowLeft, Trash2 } from "lucide-react"
...
import { EditPaymentDetailsForm } from "@/components/form/EditPaymentDetails"
import { deletePaymentDetails } from "@/action/payment-details"
import { toast } from "sonner"
import { useRouter, useSearchParams } from "next/navigation"
import Link from "next/link"

interface EditPaymentDetailsClientProps {
  initialPaymentDetails: any[]
  schemeNames: string[]
}

export function EditPaymentDetailsClient({ initialPaymentDetails, schemeNames }: EditPaymentDetailsClientProps) {
  ...
  const getAwardedCost = (payment: any) => {
    return payment.WorksDetail?.AwardofContract?.workorderdetails[0]?.Bidagency?.biddingAmount || 0
  }
  ...
  return (
    <div className="container mx-auto py-8 px-4 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Payment Details</h1>
          <p className="text-gray-600 mt-1">Manage and update payment details for works</p>
        </div>
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-purple-900">Filter Payments</h3>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-blue-600">
                  {selectedScheme === "all" ? "Total Payments" : "Filtered Payments"}
                </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-blue-100">
                {selectedScheme === "all"
                  ? "List of all unverified payment details available for editing"
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl">Edit Payment Details</DialogTitle>
          </DialogHeader>
          <div className="py-4">
            {selectedPaymentId && (() => {
              // Find the payment details
              const payment = filteredPaymentDetails.find((p) => p.id === selectedPaymentId)
              return payment ? (
                <EditPaymentDetailsForm
                  paymentDetailsId={selectedPaymentId}
                  awardedCost={getAwardedCost(payment)}
                  onSuccess={() => {
                    setOpenDialog(false)
                    setSelectedPaymentId(null)
                    router.refresh()
