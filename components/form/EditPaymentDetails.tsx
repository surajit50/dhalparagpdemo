...
import { updatePaymentDetails, getPaymentDetails } from "@/action/payment-details"
import { cn } from "@/lib/utils"

// Type guard for billType
function isValidBillType(value: string): value is "1st RA" | "2nd RA" | "3rd RA" | "Final Bill" {
  return value === "1st RA" || value === "2nd RA" || value === "3rd RA" || value === "Final Bill"
}

interface EditPaymentDetailsFormProps {
  paymentDetailsId: string
  awardedCost: number
  onSuccess: () => void
}

export function EditPaymentDetailsForm({ 
  paymentDetailsId, 
  awardedCost,
  onSuccess 
}: EditPaymentDetailsFormProps) {
  ...
  useEffect(() => {
    const loadPaymentDetails = async () => {
      try {
        setIsLoading(true)
        const result = await getPaymentDetails(paymentDetailsId)
        ...
        if (result?.paymentDetails) {
          ...
          // Calculate security deposit percentage
          const gross = pd.grossBillAmount
          const securityDeposit = pd.securityDeposit?.securityDepositAmt || 0
          const calculatedPercentage = gross > 0 ? Math.round((securityDeposit / gross) * 100) : 10
          setSecurityDepositPercentage(calculatedPercentage)

          // Type-safe billType assertion using type guard
          const billTypeValue: "1st RA" | "2nd RA" | "3rd RA" | "Final Bill" = 
            isValidBillType(pd.billType) ? pd.billType : "1st RA"

          form.reset({
            grossBillAmount: pd.grossBillAmount,
            lessIncomeTax: pd.lessIncomeTax?.incomeTaaxAmount || 0,
            lessLabourWelfareCess: pd.lessLabourWelfareCess?.labourWelfarecessAmt || 0,
            lessTdsCgst: pd.lessTdsCgst?.tdscgstAmt || 0,
            lessTdsSgst: pd.lessTdsSgst?.tdsSgstAmt || 0,
            mbrefno: pd.mbrefno,
            securityDeposit: securityDeposit,
            billPaymentDate: new Date(pd.billPaymentDate),
            workcompletaitiondate: pd.workcompletaitiondate ? new Date(pd.workcompletaitiondate) : undefined,
            eGramVoucher: pd.eGramVoucher,
            eGramVoucherDate: new Date(pd.eGramVoucherDate),
            gpmsVoucherNumber: pd.gpmsVoucherNumber,
            gpmsVoucherDate: new Date(pd.gpmsVoucherDate),
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const response = await updatePaymentDetails(paymentDetailsId, values)
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Edit basic bill details</p>
                </div>
              </div>
            ...
            <CardFooter className="px-0 pb-0 pt-4">
              ...
              >
                ...
                {isSubmitting ? "Updating..." : "Update Payment Details"}
              </Button>
            </CardFooter>
          </form>
        </Form>
